...
namespace {
    ...
    auto const mat3 = suite.test(
            ...
            [](auto check, auto &log) {
                auto const m = planet::affine::matrix3d::rotate_y(0.25f);
                auto const p = m * planet::affine::point3d{1.0f, 0.0f, 0.0f};
                log << "(x, y, z) = (" << p.x() << ", " << p.y() << ", "
                    << p.z() << ")\n";
                check(p.x()) < 1e-7f;
                check(p.x()) > -1e-7f;
    ...
    auto const tran3 = suite.test(
            "transform3d",
            [](auto check, auto &log) {
                planet::affine::transform3d t3;
                auto const p =
                        t3.into() * planet::affine::point3d{1.0f, 2.0f, 3.0f};
                log << "(x, y, z) = (" << p.x() << ", " << p.y() << ", "
                    << p.z() << ")\n";
                check(p.x()) == 1.0f;
                check(p.y()) == 2.0f;
            ...
            [](auto check, auto &log) {
                ...
                t3.rotate_x(0.25f);
                auto const p =
                        t3.into() * planet::affine::point3d{0.0f, 1.0f, 0.0f};
                log << "(x, y, z) = (" << p.x() << ", " << p.y() << ", "
                    << p.z() << ")\n";
                check(p.x()) == 0.0f;
                check(p.y()) < 1e-7f;
            ...
            [](auto check, auto &log) {
                ...
                t3.rotate_y(0.25f);
                auto const p =
                        t3.into() * planet::affine::point3d{1.0f, 0.0f, 0.0f};
                log << "(x, y, z) = (" << p.x() << ", " << p.y() << ", "
                    << p.z() << ")\n";
                check(p.x()) < 1e-7f;
                check(p.x()) > -1e-7f;
    ...
    float constexpr scale = 1.0f;
    float constexpr theta = 1.0f;
    auto const t = planet::affine::transform3d::perspective(scale, theta);
    auto const pers3 = suite.test(
