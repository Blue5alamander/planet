...
* [`planet/events/kinds.hpp`](./kinds.hpp) -- The `kinds` set, naming some subset of the four event kinds that are routed to widgets. Used to say which kinds a hard focus captures and which kinds a widget swallows -- see [Event routing](#event-routing) below.
